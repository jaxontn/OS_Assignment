                     ...
                     int ready_count, int *burst_count )
{
     ...
     if( ready_count >= ONE )/*have multiple task to choose from*/
     {
         ...
         if( !running[0].empty )
         {
             ...
             if( running[0].rt > readyQueue[highPrioIdx].rt )
             {
                 ready_count = srtfDetermineRun( readyQueue, running, 
...
int srtfDetermineRun( SRTFTask readyQueue, SRTFTask running, 
                      int ready_count, int highPrioIdx )
{
    int i;
